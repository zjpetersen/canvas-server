...
//On app startup we should load the cache
...
const addEmail = (email, fn) => {
    let rand = crypto.randomBytes(64).toString('hex');
	conn.query("INSERT INTO emails (emailAddress, active, hash) VALUES ('" + email + "', false, '" + rand + "');", function (err, result) {
		if (err) {
			console.log(err);
			fn(result, true, rand)
		} else {
			fn(result, false, rand);
		}
	});
}
...
function updateTileCache() {
    selectAllTiles(function (queryResult) {
      for (let i = 0; i < queryResult.length; i++) {
        queryResult[i].color = queryResult[i].color.toString('utf8');
      }
	  cache.updateTileCache(queryResult);
	  console.log("Updated tile cache");
    });
}
